    ...
    >
      ...
      <div class="overflow-hidden rounded-3xl bg-white shadow-2xl dark:bg-gray-900">
        ...
        <div class="max-h-[600px] overflow-y-auto bg-white dark:bg-gray-900">
          <!-- 图片轮播 -->
          <div v-if="images.length > 0" class="relative aspect-square w-full bg-gray-100 dark:bg-gray-800">
            <img
            ...
            <button
              ...
              class="absolute left-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-black/50 text-white transition-all hover:bg-black/70"
            >
            ...
            <button
              ...
              class="absolute right-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-full bg-black/50 text-white transition-all hover:bg-black/70"
            >
              <UIcon name="i-heroicons-chevron-right" class="h-5 w-5" />
            </button>

            <!-- 图片指示器 -->
            <div v-if="images.length > 1" class="absolute bottom-3 left-1/2 flex -translate-x-1/2 gap-1">
              <div
            ...
            <!-- 图片计数 -->
            <div class="absolute right-3 top-3 rounded-full bg-black/50 px-2 py-1 text-xs text-white">
          ...
          <div class="p-4">
            <!-- 标题 -->
            <div class="mb-3">
              <div v-if="!isEditing" class="text-lg font-semibold text-gray-900 dark:text-white">
                {{ editTitle }}
              </div>
              <textarea
                v-else
                ...
                rows="2"
                placeholder="输入标题..."
                class="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-lg font-semibold focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-white"
              />
            </div>

            <!-- 正文 -->
            <div class="mb-4">
              <div v-if="!isEditing" class="whitespace-pre-wrap text-sm leading-relaxed text-gray-700 dark:text-gray-300">
                {{ editContent }}
              </div>
              <textarea
                v-else
                ...
                rows="8"
                placeholder="输入正文内容..."
                class="w-full resize-none rounded-lg border border-gray-300 px-3 py-2 text-sm leading-relaxed focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-300"
              />
            </div>
            ...
            <div class="rounded-lg border border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
              <div class="flex items-center gap-3">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <p v-if="props.product.spec" class="truncate text-xs text-gray-600 dark:text-gray-400">
                    {{ props.product.spec }}
                  </p>
                  <p v-if="props.product.price" class="mt-1 text-sm font-semibold text-red-600">
                    ¥{{ props.product.price }}
                  </p>
                </div>
                <UButton
